...
from numpy import mean as np_mean, std as np_std, median as np_median
...
def get_average_cycle_length(user_id_hash):
    ...
    if cycle_lengths:
        return np_mean(cycle_lengths)
    return None

def get_period_statistics(user_id_hash):
    cycle_lengths = calculate_cycle_lengths(user_id_hash)
    if cycle_lengths:
        average_cycle_length = np_mean(cycle_lengths)
        std_dev_cycle_length = np_std(cycle_lengths)
        median_cycle_length = np_median(cycle_lengths)
...
def predict_next_period_start(user_id_hash):
    ...
    if cycle_lengths:
        ...
        avg_cycle_length = np_mean(cycle_lengths)
        std_dev_cycle_length = np_std(cycle_lengths)
